...
def correct_maximum_allowed_exercises_for_phase_component(phase_components, possible_exercises):
    exercises_for_pcs = get_exercises_for_all_pcs(possible_exercises[1:], phase_components[1:])

    # for phase_component, exercises_for_pc in zip(phase_components[1:], exercises_for_pcs):
    #     number_of_exercises_available = len(exercises_for_pc)
    #     # Correct upper bound for exercises.
    #     if phase_component["exercises_per_bodypart_workout_max"]:
    #         phase_component["exercises_per_bodypart_workout_max"] = min(number_of_exercises_available, phase_component["exercises_per_bodypart_workout_max"])
    #     else:
    #         phase_component["exercises_per_bodypart_workout_max"] = number_of_exercises_available
...
def retrieve_pc_parameters(user_workout_components, availability):
    ...
    correct_maximum_allowed_exercises_for_phase_component(parameters["phase_components"], parameters["possible_exercises"])
    return parameters
...
def exercise_initializer():
    ...
    total_time_needed = retrieve_total_time_needed(parameters["phase_components"][1:], "duration_min", "exercises_per_bodypart_workout_min")
