...
def confirm_regenerate(state):
    ...
    LogCreationAgent.agent_steps(f"\t---------Determine if the {sub_agent_focus} schedule should be regenerated---------")
    if state.get("is_regenerated", False):
        LogCreationAgent.agent_steps(f"\t---------Is Regenerated---------")
...
class BaseAgent():
    ...
    def start_node(self, state):
        LogCreationAgent.agent_introductions(f"\n=========Beginning User {self.sub_agent_title} Creation Agent=========")
        return {}
    ...
    def end_node(self, state):
        LogCreationAgent.agent_introductions(f"=========Ending User {self.sub_agent_title} Creation Agent=========\n")
        return {}
