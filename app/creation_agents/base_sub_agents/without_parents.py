...
def confirm_new_input(state):
    ...
    LogCreationAgent.agent_steps(f"\t---------Confirm there is a new {sub_agent_focus} input to be parsed---------")
...
class BaseAgentWithoutParents(BaseAgent):
    ...
    def ask_for_new_input(self, state):
        LogCreationAgent.agent_steps(f"\t---------Ask user if a new {self.sub_agent_title} can be made---------")
        result = interrupt({
            "task": f"No current {self.sub_agent_title} exists. Would you like for me to generate a {self.sub_agent_title} for you?"
        })
        user_input = result["user_input"]
        LogCreationAgent.verbose(f"Extract the {self.sub_agent_title} Goal the following message: {user_input}")
    ...
    def no_new_input_requested(self, state):
        LogCreationAgent.agent_steps(f"\t---------Abort {self.sub_agent_title} Parsing---------")
