...
def add_local_bounds_violation(violations, pc_name, item, value_name, min_value, max_value):
...
def check_schedule_validity(schedule_list):
    ...
    for schedule_item in schedule_list:
        ...
        add_local_bounds_violation(violations, pc_name, schedule_item, "reps", pc.reps_min, pc.reps_max)

        # Exercise must have a number of sets between the min and max allowed for the phase component.
        add_local_bounds_violation(violations, pc_name, schedule_item, "sets", pc.sets_min, pc.sets_max)

        # Exercise must have a rest between the min and max allowed for the phase component.
        add_local_bounds_violation(violations, pc_name, schedule_item, "rest", pc.rest_min, pc.rest_max)
        ...
        if schedule_item["intensity"] and pc.intensity_min and pc.intensity_max:
            ...
            add_local_bounds_violation(violations, pc_name, schedule_item, "intensity", pc.intensity_min, pc.intensity_max)
